...
def validate_download_args(url, user, password):
    if not url or not user or not password:
        print('Error: Server URL, user, and password are required for download mode', file=sys.stderr)
        sys.exit(1)


def pull_programs(server_url, headers):
    response = requests.get(
        server_url + "/api/programs.json?fields=id,name,userGroupAccesses[displayName],programType&paging=false", headers=headers)
...
def pull_datasets(server_url, headers):
    response = requests.get(
        server_url + "/api/dataSets.json?fields=id,name,userGroupAccesses[displayName]&paging=false", headers=headers)
    if response.status_code == 200:
    ...
    try:
        with open(output_filename+".csv", 'w', newline='') as archivo_csv:
            ...
            for program in programs:
                ...
                listOfGroupNames = get_group_names(program)
                write_csv.writerow({
                    'uid': program.get('id'),
                    'Dep_name': dep_name,
            ...
            for dataSet in dataSets:
                ...
                listOfGroupNames = get_group_names(dataSet)
                write_csv.writerow({
                    'uid': dataSet.get('id'),
                    'Dep_name': dep_name,
        ...
        print(f'Error writing {output_filename}.csv: {e}', file=sys.stderr)
        sys.exit(1)


def main():
    parser = argparse.ArgumentParser(
        description='This tool make simple convert csv with the departments to the config format or update the config departments from dhis2.')
    ...
    parser.add_argument('--convert', required=False,
                        help='Convert file. If the file is CSV, it will be converted to JSON and vice versa.')
    ...
    if args.convert:
        if args.convert.endswith('.csv'):
            convert_csv_to_json(args.convert, output_filename+".json")
        else:
            convert_json_to_csv(args.convert, output_filename+".csv")
    else:
        validate_download_args(args.url, args.user, args.password)
