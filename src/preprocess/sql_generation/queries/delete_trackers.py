...
def generate_delete_tracker_rules(trackers, data_elements, org_units, org_unit_descendants,
    ...
    sql_query = """DELETE FROM {programstageinstance} where programstageid in 
     (select programstageid from programstage where programid in 
      (select programid from program where uid in {uids})) """.format(programstageinstance=get_event_table_name(),
                                                                      uids=sql_all)

    if sql_data_elements != "":
        has_rule = True
        sql_data_elements = sql_data_elements.replace("(", "").replace(")", "")
        sql_query = """ update {programstageinstance} set eventdatavalues = eventdatavalues - {remove_data_elements_uid}  where eventdatavalues ? 
         {where_data_elements_uid} and """.format(programstageinstance=get_event_table_name(),
                                                  remove_data_elements_uid=sql_data_elements,
        ...
        if sql_trackers != "":
            sql_query = sql_query + """ programstageid in (select programstageid from programstage where programid in 
             (select programid from program where uid in {tracker_program_uid})) and""".format(
                tracker_program_uid=sql_trackers)
        else:
            sql_query = sql_query + """ programstageid in (select programstageid from programstage where programid in 
             (select programid from program where uid in {tracker_program_uid})) and """.format(
                tracker_program_uid=sql_all)

    elif sql_trackers != "":
        has_rule = True
        sql_query = sql_query + """ and dataelementid IN (
    ...
    if sql_org_units != "":
        ...
        sql_query = sql_query + """  and organisationunitid in (select organisationunitid from organisationunit where
          uid in {org_units}) and """.format(org_units=sql_org_units)

    if sql_org_unit_descendants != "":
        has_rule = True
        sql_query = sql_query + """ and organisationunitid in (select organisationunitid from organisationunit where 
         path like (select concat(path,'/%') from organisationunit where uid in {org_units})) and """.format(
            org_units=sql_org_unit_descendants)
...
def delete_all_tracker_programs(trackers, f):
    ...
    """.format(trackedentityinstanceid=get_enrollment_identifier_name(), programinstance=get_enrollment_table_name(),
    ...
    inner join programstage ps on ps.programstageid=psi.programstageid 
    inner join program p on p.programid=ps.programid 
    where p.uid in {tracker_uids});""".format(programstageinstance=get_event_table_name(),
    ...
    """.format(programinstance=get_enrollment_table_name(), trackedentityinstance=get_tracker_table_name(), trackedentityinstanceid=get_tracker_identifier_name()))
