...
def show_data_summary(f):
    ...
    count_events_gropued_per_program(f)
...
def count_datavalues_grouped_per_dataset(f):
    ...
    write(f, "SELECT 'Database summary - data values per data set' AS info;\n")
    write(f, """
SELECT
...
def count_events_gropued_per_program(f):
    write(f, "\n-- Events per program\n")
    write(f, "SELECT 'Database summary - events per program' AS info;\n")
    write(f, """
SELECT
...
def count_enrollments_grouped_per_program(f):
    ...
    write(f, "SELECT 'Database summary - enrollments per program' AS info;\n")
    write(f, """
SELECT
    p.uid  AS program_uid,
    p.name AS program_name,
...
def count_tracked_entities_grouped_per_program(f):
    ...
    write(f, "SELECT 'Database summary - tracked entity instances per program' AS info;\n")
    write(f, """
SELECT
    p.uid  AS program_uid,
    p.name AS program_name,
